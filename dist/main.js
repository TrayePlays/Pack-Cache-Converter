...
function rename() {
    ...
    fs.readdir(dir, {}, (err, files) => {
        ...
        for (const file of files) {
            fs.readFile(path.join(dir, file, "manifest.json"), (err, data) => {
                if (err) {
                    console.error(err);
                    return;
                }
                ...
                try {
                    ...
                    console.error(e);
                    fs.readFile(path.join(dir, file, "texts", "en_US.lang"), (err, data) => {
                        if (err) {
                            console.error(err);
                            return;
                        }
                ...
                name = newName;
                fs.rename(path.join(dir, file), path.join(dir, name), (err) => {
                    if (err) {
                        console.error(err);
                        return;
                    }
